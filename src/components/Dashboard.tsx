...
export const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch basic stats
      const [entriesRes, categoriesRes, shopsRes] = await Promise.all([
        supabase.from('goods_damaged_entries').select('*'),
      ...
      // Fetch shop stats
      const { data: shopStatsData } = await supabase
        .from('goods_damaged_entries')
        .select(`
          shop_id,
          shops (name)
      ...
      if (shopStatsData) {
        ...
        setShopStats(
          Array.from(shopMap.entries()).map(([name, count]) => ({ name, count }))
        );
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Distribution of damaged goods by category</CardDescription>
          </CardHeader>
          <CardContent>
